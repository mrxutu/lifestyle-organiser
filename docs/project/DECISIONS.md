...
A dated record of non-obvious product and technical decisions made during the build. Entries preserve the context that existed when each decision was taken; they are not rewritten to resemble current state.

For current capabilities and architecture, use `docs/project/OVERVIEW.md`. Current code, configuration, `prisma/schema.prisma` and later explicitly superseding entries take precedence over older statements. Temporary verification and deployment-status notes describe the time of the entry only; production status must always be checked through the current release procedure.

Materially superseded entries are labelled below. Add future decisions only when they explain a durable choice that is not already obvious from the code or current documentation.
...
**Status: Superseded by “Event attendees un-deferred (user assignment)” below.**
...
**Status: Partly superseded. The current Watchlist uses a card-based responsive list and includes rating and viewer filters; see `docs/project/OVERVIEW.md`.**
...
**Status: Partly superseded. A later migration added `SUPER_ADMIN`, and authentication hardening replaced raw stored tokens with `tokenHash`.**

Schema-only step ahead of the admin page and password-reset flow. Added `Role { ADMIN, MEMBER }` enum and `role Role @default(MEMBER)` on `User`. Added `PasswordResetToken { id, token (unique), userId → User, expiresAt, createdAt }` with the inverse `passwordResetTokens` relation on `User`. Migration: `20260804092844_add_user_role_and_password_reset_token`, applied to the local development database at the time of this entry. Existing user `p@ulcozens.com` set to `role: ADMIN` via a one-off script against the development database (all other users stayed `MEMBER`, the default). No UI/route changes in this step.
...
**Status: Superseded by “Authentication hardening” below. The current minimum is 12 characters with a 72-byte bcrypt limit and common-password denylist.**
...
**Status: Partly superseded. `BookSource` is now household-owned, and Books later gained ratings and cover images.**
...
`app/reset-password/reset-password-form.tsx`: added a "Confirm password" field, checked against the first on submit (client-side only — the API still just takes one `password` field) with a "Passwords don't match." error, and a client-side check that surfaces the existing 8-character minimum as a proper error message rather than relying solely on the `minLength` HTML attribute. Also added static helper text under the password field ("Must be at least 8 characters.") stating the policy up front — matches the min-8 rule set when the reset flow was first built (see the 1c entry above), not a new policy.

**Status: The confirmation-field decision remains current; the 8-character policy was superseded by “Authentication hardening” below.**

**2026-08 — Follow-up: link generation switched to a fixed `HOSTNAME` env var**
Both password emails previously built their link from the inbound request (`request.nextUrl.origin`, threaded through `createUser`/`sendPasswordResetEmail`/`sendNewUserSetPasswordEmail` as an `origin` param) — Paul flagged this made the generated link unreliable when testing. Added `HOSTNAME` to `.env` (`http://localhost:3000` locally; production requires the corresponding absolute HTTPS origin) and switched `lib/password-reset.ts`'s `sendPasswordSetupEmail` helper to build the reset URL from `process.env.HOSTNAME` unconditionally, no request-derived fallback. Confirmed with Paul: always use `HOSTNAME`, not "override with fallback to request origin."

Removed the now-dead `origin` parameter end to end: `sendPasswordResetEmail(user)`, `sendNewUserSetPasswordEmail(user)`, and `createUser(input)` (in `lib/admin-users.ts`) all dropped it, and the two callers (`app/api/auth/forgot-password/route.ts`, `app/api/admin/users/route.ts`) stopped passing `request.nextUrl.origin`. Also caught and fixed the initial `.env` value (`Localhost:3000`, then `localhost:3000`) — missing the `http://` scheme, which `new URL(path, base)` needs to parse `base` as an actual origin rather than a bogus opaque-scheme URL.

**2026-08 — Authentication hardening**
Authentication throttling is stored in PostgreSQL rather than process memory so it is shared across Vercel instances and deployments. Login uses account and trusted-source-IP limits; forgot-password and reset-password use their own account/token and IP policies. Limiter identifiers are action-scoped HMAC-SHA-256 values derived from `AUTH_RATE_LIMIT_SECRET` (or the existing `AUTH_SECRET` fallback), so raw emails, IP addresses, passwords and reset tokens are never stored in the limiter table. Database-native `INSERT ... ON CONFLICT DO UPDATE` makes concurrent increments atomic without serializable transactions.

Password-reset tokens remain 256-bit random bearer values in email links, but only their SHA-256 hashes are stored. One unique token row is allowed per user and issuance uses an atomic upsert, so concurrent requests deterministically leave exactly one valid link without the earlier serializable-transaction/P2034 failure mode. Consumption atomically claims a valid token with `DELETE ... RETURNING` inside the same transaction as the password update, deletes any remaining tokens for that user, and increments `User.authVersion`. Existing reset links are deliberately invalidated by the migration.

JWTs now carry `authVersion` and are checked against the database on authenticated requests. Missing or stale versions are rejected, intentionally causing a one-time logout at deployment. Password reset and both account disable and re-enable increment the version, preventing old device sessions from becoming valid again. The reset page sends a `no-referrer` policy.

The shared password policy is a minimum of 12 characters, a maximum of 72 UTF-8 bytes while bcrypt remains in use, and a small local common-password denylist. Spaces and Unicode are supported and there are no composition rules. Existing password hashes remain valid. Login, forgot-password, admin user creation and seed user creation consistently trim and lowercase email addresses; the migration normalizes existing addresses after a required collision check and adds a database check constraint.

**2026-08 — `PasswordResetToken` → `User` FK set to cascade delete**
Paul hit `update or delete on table "User" violates RESTRICT setting of foreign key constraint "PasswordResetToken_userId_fkey"` deleting a user from `/admin` after already clearing their events/recipes — the `PasswordResetToken.user` relation had no `onDelete` behavior, so Postgres defaulted to `RESTRICT` and any leftover token row (even an expired, harmless one — e.g. the row created but never consumed when the user was originally invited via `createUser`) blocked the delete. Unlike Events/Recipes, which `deleteUser` deliberately blocks on via `UserHasContentError` pending manual reassignment, a reset token has no meaning once its user is gone, so cascade rather than block is correct here — confirmed with Paul (chose schema-level `onDelete: Cascade` over an app-level `deleteMany` in `deleteUser()`, so any future deletion path gets the same behavior for free, not just the admin UI). Schema change: `user User @relation(fields: [userId], references: [id], onDelete: Cascade)`, migration `20260806091737_password_reset_token_cascade_delete`, applied to the local development database at the time of this entry. Current production migration state must be checked through `docs/operational/RELEASE.md` rather than inferred from this log.

**2026-08 — Generated Prisma client remains committed**
Prisma 7 requires an explicit generator output path, but it does not require generated files to be committed. The repository intentionally retains `generated/prisma/` in version control because its install, build and test commands do not automatically run `prisma generate`, so the tracked client currently provides reliable local and deployment builds. The generated tree is reproducible from `prisma/schema.prisma` and the installed Prisma version and contains no hand-authored customisation. Schema or Prisma-version changes must include a deliberate `prisma generate` and review of the resulting generated diff. Reconsider this decision if the build or deployment pipeline later gains a reliable generation stage.
